...
public class WeatherServer {
    ...
    private static final String BASE_URL = "http://localhost:8080/";// CR: Should be coming from a central configuration place such as a constants/properties file
    ...
    public static void main(String[] args) {
        try {
            ...
            resourceConfig.register(RestWeatherQueryEndpoint.class);// CR: We can add option of Gzip compression here for optimizing response
            final HttpServer server = GrizzlyHttpServerFactory.createHttpServer(URI.create(BASE_URL), resourceConfig, false);

            Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
                @Override
                public void run() {
                    server.shutdownNow();// CR: We can aim for a more graceful shutdown of the server
                }
            }));

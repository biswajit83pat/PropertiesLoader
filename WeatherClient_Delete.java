...
public class WeatherClient {
    ...
    public WeatherClient() {
        ...
        query = client.target(BASE_URI + "/queries");
        collect = client.target("http://localhost:8080/collectors");
    }
    ...
    public void delete() {
        try{WebTarget path = collect.path("/airports/BOS");
        Response post = path.request().delete();}catch(Exception e){e.printStackTrace();}
    }
    
    
    public void queryForAirports() {
        WebTarget path = collect.path("/airports/BOS");
        Response response = path.request().get();
        System.out.println("queryforAirports.get:" + response.readEntity(String.class));
    }

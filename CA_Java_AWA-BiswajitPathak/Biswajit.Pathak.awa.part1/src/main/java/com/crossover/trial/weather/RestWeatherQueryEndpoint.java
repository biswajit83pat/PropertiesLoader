 ...
 */
@Path("/query")// CR: Instead of verb 'query' we can use plural noun such as 'queries'
public class RestWeatherQueryEndpoint implements WeatherQueryEndpoint {
     ...
     */
    public static Map<AirportData, Integer> requestFrequency = new HashMap<AirportData, Integer>(); // CR: Should use ConcurrentHashMap for thread Safety and it should be ensured that AirportData is immutable

    public static Map<Double, Integer> radiusFreq = new HashMap<Double, Integer>();// CR: should use ConcurrentHashMap for thread safety
    ...
    public String ping() {
        ...
        for (AtmosphericInformation ai : atmosphericInformation) {
            ...
            if (ai.getCloudCover() != null
                ...
                if (ai.getLastUpdateTime() > System.currentTimeMillis() - 86400000) {// CR: It will filter out more objects if this check is implemented before null checks
                    datasize++;
                }
            }
        }
    ...
    public Response get(@PathParam("iata") String iata, @PathParam("radius") String radiusString) {
        ...
        // CR: Should be marked inside synchronized block
     ...
     */
    public void updateRequestFrequency(String iata, Double radius) {// CR: Should be marked as private since used only inside this class
     ...
     */
    public static AirportData findAirportData(String iataCode) {// CR: There is a scope to keep it in a central class as it is re-usable by 2 classes
     ...
     * @return airport data or null if not found
     */
    public static int getAirportDataIdx(String iataCode) {// CR: There is a scope to keep it in a central class as it is re-usable by 2 classes
     ...
     */
    public double calculateDistance(AirportData ad1, AirportData ad2) {// CR: Should be marked as private and synchronized since used only inside this class
        double deltaLat = Math.toRadians(ad2.latitude - ad1.latitude);// CR: Should use public accessors
        double deltaLon = Math.toRadians(ad2.longitude - ad1.longitude);// CR: Should use public accessors
        double a =  Math.pow(Math.sin(deltaLat / 2), 2) + Math.pow(Math.sin(deltaLon / 2), 2)
                * Math.cos(ad1.latitude) * Math.cos(ad2.latitude);// CR: Should use public accessors
